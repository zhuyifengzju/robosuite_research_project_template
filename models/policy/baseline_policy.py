...
class BCTransformerPolicyRGB(BasePolicy):
    ...
    def encode_fn(self, data):
        ...
        latent_outputs = []
        for img_name in self.img_names:
            latent_outputs.append(self.spatial_projection_dict[img_name](self.rgb_encoder_dict[img_name](data["obs"][img_name])))
    ...
    def forward(self, data):
        data = self.process_input_for_training(data)
        self.aug_out = self.get_aug_output_dict(self.img_aug(self.get_img_tuple(data)))
        for img_name in self.img_names:
            data[img_name] = self.aug_out[img_name]
...
class BCTransformerPolicyRGBD(BasePolicy):
    def __init__(self,
                 policy_cfg,
                 shape_meta):
        super().__init__(policy_cfg,
        ...
        policy_cfg.translation_aug.network_kwargs["input_shape"] = self.input_img_shapes[0]
        translation_aug = eval(policy_cfg.translation_aug.network)(**policy_cfg.translation_aug.network_kwargs)    
        # For RGBD augmentation, we want to apply color augmentation to RGB only, but translation augmentation to RGBD.
        # self.img_aug = DataAugGroup([color_aug, translation_aug])
